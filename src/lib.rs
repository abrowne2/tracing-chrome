...
impl FlushGuard {
    ...
    /// Starts a new trace; if one was already running it stops the current one.
    ///
    ...
    /// 
    pub fn stop_capture(&self, writer: Option<Box<dyn Write + Send>>) {
        if let Some(handle) = self.handle.take() {
        ...
        let handle = std::thread::spawn(move || {
            ...
            for msg in rx {
                ...
                let (ph, ts, callsite, id) = match &msg {
                    ...
                    Message::Flush | Message::Drop | Message::StartCapture(_) | Message::StopCapture => {
